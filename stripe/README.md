...
This store is a static site on GitHub Pages, so there is no server to hold a
...
does every store listed in `STORES`. It prints one block per store, ready to paste — add `--write` and it edits
